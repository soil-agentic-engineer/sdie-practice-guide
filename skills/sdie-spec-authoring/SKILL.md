...
- 产出物须符合 `spec-guide.md` 的方法论与红线；须按 `spec-dir-structure` 的目录约定落位。
- **绝不代签红线字段**（见下方"停止条件"）。
