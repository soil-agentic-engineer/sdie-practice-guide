...
> 正文结构依据 `spec-guide.md` §4.1；方法论标注见 §8.2。

## 1. 背景与目标（Business Goal）
   - 方法论：Impact Mapping 第 1 层——Goal（Gojko Adzic，权威推荐）。
   - 写法：每条目标用 SMART 描述，并赋编号 `GOAL-x`（x 从 1 递增），供 §4 Impact 精确引用。
   - 约束：单 Goal 与多 Goal 均适用；多 Goal 时每条独占一行编号。
...
## 4. 行为影响（Impacts）
   ...
   - 编号：每条 Impact 赋 `IMP-x`（x 从 1 递增）；"关联 Goal"列填 §1 的 `GOAL-x` 编号，**禁止用自然语言或"同上"引用**（多 Goal 场景下会断链）。
   - 归属：一条 Impact 可关联多个 Goal，用 `GOAL-1, GOAL-2` 逗号分隔；无对应 Goal 的 Impact 不允许存在（Gate 1 退回）。
...
> **用途**：Gate 1 前由 PM 填写，供 QA/Reviewer 校验「每个 Goal 至少有一条 Impact 支撑、每条 Impact 可反查到 Goal」。
> **规则**：行=Goal（§1），列=Actor（§3），格填 `IMP-x`；空格表示该 Goal 下该 Actor 无 Impact（允许）。
...
> 方法论文献：MoSCoW — Dai Clegg (1994) / DSDM；KANO — Noriaki Kano (1984) "Attractive Quality and Must-Be Quality"。操作细节见 `spec-guide.md` §5.2 与 §8。
...
> `所属版本` 与 User Story Map 的 `release` 横线、`spec-guide.md` §6.3 的 SemVer 对齐：`MVP=v1.0.0`、后续迭代=`v1.1.0`、`deferred`=剔除。
...
## 版本历史（方案 A，依据 spec-guide.md §6.5）

> 不进元信息七字段（七字段不变）；版本号遵循 §6.3 SemVer。权威溯源见 `spec-guide.md §6.5`。

| 版本 | 日期 | 变更摘要 | 关联 ADR |
|------|------|----------|----------|
| 1.0.0 | 2026-08-08 | 初始 baseline：背景/范围/用户/验收/优先级齐备，过 Gate 1 | — |
| 1.1.0 | 2026-08-12 | MINOR：补充"游客结账"用户故事（向后兼容新增） | ADR-0012 |
| 1.2.0 | 2026-08-12 | MINOR：新增 §4 行为影响（Impacts）层，原 §4~§7 顺延为 §5~§8；功能清单加"对应 Impact"列；§1 Goal 加 `GOAL-x` 编号、§4 改用 ID 引用（替代"同上"）、新增 §4.1 Goal→Impact 校验矩阵（向后兼容） | — |
